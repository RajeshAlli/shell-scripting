...
CREATE() {
  ...
  if [ $COUNT -eq 0 ]; then
    aws ec2 run-instances --image-id ami-0bb6af715826253bf --instance-type t2.micro --security-group-ids sg-0a7667be8c4e9953e --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$1}]" | jq &>/dev/null
  else
