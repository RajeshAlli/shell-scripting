...
CREATE() {
  ...
  if [ $COUNT -eq 0 ]; then
    aws ec2 run-instances --launch-template LaunchTemplateId=lt-04ed340d572610720,Version=2 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$1}]" "ResourceType=spot-instances-request,Tags=[{Key=Name,Value=$1}]" | jq &>/dev/null
  else
  ...
  aws route53 change-resource-record-sets --hosted-zone-id Z08912592O1XZJ4D3WHHT --change-batch file:///tmp/record.json | jq  &>/dev/null
}
